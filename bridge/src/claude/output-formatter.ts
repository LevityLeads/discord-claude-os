...
export function splitMessage(text: string, maxLength = MAX_DISCORD_LENGTH): string[] {
  const chunks: string[] = [];
  let remaining = text;
  ...
  while (remaining.length > 0) {
    ...
    // Try to split at the last newline before the boundary
    let splitAt = remaining.lastIndexOf("\n", maxLength);
    if (splitAt === -1 || splitAt < maxLength / 2) {
      // Fall back to the last space before the boundary to avoid splitting mid-word
      const spaceAt = remaining.lastIndexOf(" ", maxLength);
      splitAt = spaceAt > maxLength / 2 ? spaceAt : maxLength;
    }
