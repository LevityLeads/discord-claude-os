...
export async function handleButtonInteraction(
  ...
  // Handle V2 underscore-format stop buttons (stop_channelId) and completed buttons
  // These exist on messages created before the format was standardised to colons
  if (customId.startsWith("stop_")) {
    ...
    try {
      ...
      try {
        await interaction.reply({ content: L("⏹️ Task has been stopped.", "⏹️ 작업이 중지되었습니다."), ephemeral: true });
      } catch {
    ...
    if (!stopped) {
      try {
        ...
        // Stale interaction, ignore
      }
    }
    return;
  }

  if (customId === "completed_noop" || customId === "completed") {
    try {
  ...
  try {
    ...
    try {
      await interaction.reply({ content: labels[decision], ephemeral: true });
    } catch {
      // Interaction token expired, nothing we can do
    }
  }
