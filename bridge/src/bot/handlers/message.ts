...
export async function handleMessage(message: Message): Promise<void> {
  // Ignore bots, DMs, and system messages (thread starter notifications, etc.)
  if (message.author.bot || !message.guild || message.system) return;

  const isThread = message.channel.isThread();
  if (isThread) {
    console.log(`[thread-debug] Thread message: channelId=${message.channelId}, parentId=${message.channel.parentId}, name=${message.channel.name}, content="${message.content.slice(0, 50)}"`);
  }

  // Check if channel is registered (threads inherit parent channel's project)
  ...
  if (!project && isThread) {
    ...
    if (parentId) {
      ...
      if (parentProject) {
        // Auto-register the thread as a project inheriting from its parent
        // This satisfies the sessions table foreign key constraint
        registerProject(message.channelId, parentProject.project_path, parentProject.guild_id);
        project = getProject(message.channelId);
        if (isThread) {
          console.log(`[thread-debug] Auto-registered thread ${message.channelId} -> ${parentProject.project_path}`);
        }
      }
    }
  }
  ...
  if (!project) {
    if (isThread) console.log(`[thread-debug] No project found, returning`);
    return;
  }
  ...
  if (isThread) {
    console.log(`[thread-debug] About to call sendMessage, prompt length=${prompt.length}, project=${project.project_path}`);
  }

  // Thread context: tell Claude which thread it's in so it stays focused
  if (isThread) {
    const threadName = message.channel.name;
    const parentName = message.channel.parent?.name ?? "unknown";
    prompt = `[Thread: "${threadName}" in #${parentName}. Stay focused on this thread's topic.]\n\n${prompt}`;
  }
